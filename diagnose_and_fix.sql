-- Diagnostic and Fix Script for Unique Tickets System
...
AND table_name IN ('unique_tickets', 'check_ins', 'ticket_purchases')
ORDER BY table_name, ordinal_position;

-- 2. Drop and recreate tables if they exist with wrong structure
DROP TABLE IF EXISTS check_ins CASCADE;
DROP TABLE IF EXISTS unique_tickets CASCADE;

-- 3. Create unique_tickets table with correct structure
CREATE TABLE unique_tickets (
...
-- 4. Create check_ins table with correct structure
...
ALTER TABLE unique_tickets 
ADD CONSTRAINT fk_unique_tickets_purchase_id 
FOREIGN KEY (purchase_id) REFERENCES ticket_purchases(purchase_id) ON DELETE CASCADE;

ALTER TABLE check_ins 
ADD CONSTRAINT fk_check_ins_ticket_id 
FOREIGN KEY (ticket_id) REFERENCES unique_tickets(ticket_id) ON DELETE CASCADE;
...
-- 6. Add indexes
CREATE INDEX idx_unique_tickets_purchase_id ON unique_tickets(purchase_id);
...
-- 10. Test the setup
SELECT 'Testing ticket ID generation:' as test_step;
...
SELECT 'Verifying tables exist:' as test_step;
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('unique_tickets', 'check_ins');

SELECT 'Setup complete! You can now create sales with unique tickets.' as status; 